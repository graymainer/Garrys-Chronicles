...
--lua_run is size shaming because it is being handed the currentvalue of the entity, not the parameters of the output. (VERIFIED!)
--This is also why none of the other outputs aren't working, like setAnimation. we're giving it our currentvalue for some reason.
--make sure that in the keyvalues section we dont normalize the key name. If we normalize the key name before we storeoutputs, it will cause them to fail, since they're caps senstitive.
